...
import { Search, LandPlot, MapPin, Users, Phone, Globe } from "lucide-react";
...
const GolfCoursesPage = () => {
  ...
  useEffect(() => {
    fetch("/data/golfCourse.json")
      ...
      .catch((err) => console.error("Failed to load golf course data:", err));
  }, []);

  const filteredNames = golfData.filter((entry) =>
  ...
  const validCoordinates = visibleList.filter(
    (course) => course.latitude && course.longitude
  );
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-6 max-w-4xl">
            ...
            <p className="text-xl md:text-2xl max-w-2xl mx-auto leading-relaxed">
              Discover Michigan's premier golf courses, from championship
              layouts to scenic public courses.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="text-center mb-12">
            <div className="flex justify-center items-center space-x-8 mb-8">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500">Total Golf Courses</div>
              </div>
              <div className="text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
            {featured.map((course) => (
              <div
                key={course.name}
                className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-green-100 hover:border-green-200"
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                    {course.nearestCity || "Location TBD"}
                  </p>
                  {course.website && (
                    <Link
                      href={course.website}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="order-1 lg:order-2">
                  ...
                  <div className="mt-2 text-sm text-gray-500">
                    {filteredNames.length === 0 ? (
                      <span className="text-red-500">
                        No golf courses found matching "{searchTerm}"
                      </span>
                    ) : (
                      <span>Found {filteredNames.length} golf courses</span>
                    )}
                  </div>
                )}
              </div>
                  ...
                  >
                    <div className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-200 border border-gray-200 hover:border-green-300 hover:bg-green-50">
                      <div className="flex items-center">
                        <LandPlot className="w-4 h-4 text-green-500 mr-3 group-hover:text-green-600 transition-colors" />
              ...
              {!searchTerm && filteredNames.length > visibleCount && (
                <button
                  className="mt-6 w-full py-3 bg-gradient-to-r from-green-500 to-blue-500 text-white rounded-xl hover:from-green-600 hover:to-blue-600 transition-all duration-200 shadow-md hover:shadow-lg"
                  onClick={() => setVisibleCount(golfData.length)}
                >
          ...
          <div className="text-center mt-16 pt-8 border-t border-gray-200">
            <p className="text-sm text-gray-500">
              Explore Michigan's premier golf destinations, from championship
              courses to scenic public layouts.
            </p>
          </div>
        </div>
      </div>
